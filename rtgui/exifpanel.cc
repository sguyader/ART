...
ExifPanel::ExifPanel(): idata(nullptr), pl_(nullptr)
{
    ...
    Gtk::CellRendererText *render_txt =
    ...
    Gtk::TreeView::Column *viewcolv =
        Gtk::manage(new Gtk::TreeView::Column("Value"));
    Gtk::CellRendererText *render_txtv =
        Gtk::manage(new Gtk::CellRendererText());
    ...
    setExpandAlignProperties(buttons1, true, false, Gtk::ALIGN_FILL,
                             Gtk::ALIGN_CENTER);

    const auto addbtn = [&](const char *cssclass, const Glib::ustring &tip,
                            const Glib::ustring &icon1,
                            const Glib::ustring &icon2 =
                                Glib::ustring()) -> Gtk::Button * {
        ...
        setExpandAlignProperties(b, true, true, Gtk::ALIGN_FILL,
                                 Gtk::ALIGN_FILL);
    ...
    activate_none_ =
        addbtn("MiddleH", "EXIFPANEL_ACTIVATE_NONE_HINT", "box.svg");
    add = addbtn("MiddleH", "EXIFPANEL_ADDEDIT", "edit.svg");
    reset = addbtn("MiddleH", "EXIFPANEL_RESETHINT", "undo.svg", "redo.svg");
    resetAll = addbtn("Right", "EXIFPANEL_RESETALLHINT", "undo-all.svg",
                      "redo-all.svg");
    ...
    reset->signal_clicked().connect(
        sigc::mem_fun(*this, &ExifPanel::resetPressed));
...
void ExifPanel::write(ProcParams *pp)
{
    ...
    pp->metadata.exifKeys.assign(cur_active_keys_.begin(),
                                 cur_active_keys_.end());
                       ...
                       bool activatable)
{
    ...
    bool active =
        activatable && (all_keys_active() ||
                        cur_active_keys_.find(key) != cur_active_keys_.end());
...
void ExifPanel::refreshTags()
{
    ...
    try {
        ...
        struct KeyLt {
            KeyLt()
                : order_{
                      {"Exif.GPSInfo", 0}, {"Exif.Photo", 1}, {"Exif.Image", 2}}
            {
            }
...
void ExifPanel::setKeyActive(Gtk::CellRenderer *renderer,
                             const Gtk::TreeModel::iterator &it)
{
    auto row = *it;
    Gtk::CellRendererToggle *toggle =
        static_cast<Gtk::CellRendererToggle *>(renderer);
    if (row[exifColumns.is_group]) {
...
void ExifPanel::onExifTreeClick(GdkEventButton *event)
{
    ...
    int cell_x;
    int cell_y;
    if (exifTree->get_path_at_pos(event->x, event->y, pth, col, cell_x,
                                  cell_y) &&
...
void ExifPanel::onExifRowExpanded(const Gtk::TreeModel::iterator &it,
...
void ExifPanel::onExifRowCollapsed(const Gtk::TreeModel::iterator &it,
                                   const Gtk::TreeModel::Path &path)
{
    auto row = *it;
    if (row[exifColumns.is_group]) {
...
void ExifPanel::setExifTagValue(Gtk::CellRenderer *renderer,
                                const Gtk::TreeModel::iterator &it)
{
    auto row = *it;
...
Glib::ustring get_focallen(const Glib::ustring &val)
{
    Glib::MatchInfo m;
    auto re =
        Glib::Regex::create("^ *([0-9.]+) *(mm)? *$", Glib::REGEX_CASELESS);
    if (re->match(val, m)) {
        auto s = m.fetch(1);
        return to_fraction(s);
    } else {
...
Glib::ustring get_expcomp(const Glib::ustring &val)
{
    Glib::MatchInfo m;
    auto re =
        Glib::Regex::create("^ *(-?[0-9.]+) *(EV)? *$", Glib::REGEX_CASELESS);
    if (re->match(val, m)) {
        auto s = m.fetch(1);
        return to_fraction(s);
    } else {
...
void ExifPanel::onEditExifTagValue(const Glib::ustring &path,
                                   const Glib::ustring &val)
{
    ...
    if (good) {
        ...
        pl_->error(
            Glib::ustring::compose(M("ERROR_MSG_METADATA_VALUE"), key, value));
    }
}
