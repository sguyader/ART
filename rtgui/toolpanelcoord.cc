...
ToolPanelCoordinator::ToolPanelCoordinator(bool batch)
    : ipc(nullptr), favoritePanelSW(nullptr), hasChanged(false),
      editDataProvider(nullptr)
{
    ...
    // So Demosaic, Line noise filter, Green Equilibration, Ca-Correction
    // (garder le nom de section identique!) and Black-Level will be moved in a
    // "Bayer sensor" tool, and a separate Demosaic and Black Level tool will be
    // created in an "X-Trans sensor" tool

    // X-Trans demozaic methods: "3-pass (best), 1-pass (medium), fast"
    // Mettre  jour les profils fournis pour inclure les nouvelles section Raw,
    // notamment pour "Default High ISO" Valeurs par dfaut:
    //     Best -> low ISO
    //     Medium -> High ISO
    ...
    if (favoriteCount > 0) {
        ...
        favoritePanel->pack_start(*Gtk::manage(new Gtk::HSeparator),
                                  Gtk::PACK_SHRINK, 0);
        favoritePanel->pack_start(*vbPanelEnd[0], Gtk::PACK_SHRINK, 4);
    }

    exposurePanelSW->add(*exposurePanel);
    exposurePanel->pack_start(*Gtk::manage(new Gtk::HSeparator),
                              Gtk::PACK_SHRINK, 0);
    exposurePanel->pack_start(*vbPanelEnd[1], Gtk::PACK_SHRINK, 4);

    detailsPanelSW->add(*detailsPanel);
    detailsPanel->pack_start(*Gtk::manage(new Gtk::HSeparator),
                             Gtk::PACK_SHRINK, 0);
    detailsPanel->pack_start(*vbPanelEnd[2], Gtk::PACK_SHRINK, 4);

    colorPanelSW->add(*colorPanel);
    colorPanel->pack_start(*Gtk::manage(new Gtk::HSeparator), Gtk::PACK_SHRINK,
                           0);
    colorPanel->pack_start(*vbPanelEnd[3], Gtk::PACK_SHRINK, 4);

    // advancedPanelSW->add       (*advancedPanel);
    // advancedPanel->pack_start (*Gtk::manage (new Gtk::HSeparator),
    // Gtk::PACK_SHRINK, 0); advancedPanel->pack_start (*vbPanelEnd[6],
    // Gtk::PACK_SHRINK, 0);

    localPanelSW->add(*localPanel);
    localPanel->pack_start(*Gtk::manage(new Gtk::HSeparator), Gtk::PACK_SHRINK,
                           0);
    localPanel->pack_start(*vbPanelEnd[6], Gtk::PACK_SHRINK, 0);

    effectsPanelSW->add(*effectsPanel);
    effectsPanel->pack_start(*Gtk::manage(new Gtk::HSeparator),
                             Gtk::PACK_SHRINK, 0);
    effectsPanel->pack_start(*vbPanelEnd[7], Gtk::PACK_SHRINK, 0);

    transformPanelSW->add(*transformPanel);
    transformPanel->pack_start(*Gtk::manage(new Gtk::HSeparator),
                               Gtk::PACK_SHRINK, 0);
    transformPanel->pack_start(*vbPanelEnd[4], Gtk::PACK_SHRINK, 4);

    rawPanelSW->add(*rawPanel);
    rawPanel->pack_start(*Gtk::manage(new Gtk::HSeparator), Gtk::PACK_SHRINK,
                         0);
    rawPanel->pack_start(*vbPanelEnd[5], Gtk::PACK_SHRINK, 0);

    toiF = Gtk::manage(new TextOrIcon("star.svg", M("MAIN_TAB_FAVORITES"),
                                      M("MAIN_TAB_FAVORITES_TOOLTIP")));
    toiE = Gtk::manage(new TextOrIcon("exposure.svg", M("MAIN_TAB_EXPOSURE"),
                                      M("MAIN_TAB_EXPOSURE_TOOLTIP")));
    toiD = Gtk::manage(new TextOrIcon("detail.svg", M("MAIN_TAB_DETAIL"),
                                      M("MAIN_TAB_DETAIL_TOOLTIP")));
    toiC = Gtk::manage(new TextOrIcon("color-circles.svg", M("MAIN_TAB_COLOR"),
                                      M("MAIN_TAB_COLOR_TOOLTIP")));
    toiW = Gtk::manage(new TextOrIcon("atom.svg", M("MAIN_TAB_ADVANCED"),
                                      M("MAIN_TAB_ADVANCED_TOOLTIP")));
    toiT = Gtk::manage(new TextOrIcon("transform.svg", M("MAIN_TAB_TRANSFORM"),
                                      M("MAIN_TAB_TRANSFORM_TOOLTIP")));
    toiR = Gtk::manage(new TextOrIcon("bayer.svg", M("MAIN_TAB_RAW"),
                                      M("MAIN_TAB_RAW_TOOLTIP")));
    toiM = Gtk::manage(new TextOrIcon("metadata.svg", M("MAIN_TAB_METADATA"),
                                      M("MAIN_TAB_METADATA_TOOLTIP")));
    toiL = Gtk::manage(new TextOrIcon("local-edit.svg", M("MAIN_TAB_LOCAL"),
                                      M("MAIN_TAB_LOCAL_TOOLTIP")));
    toiFx = Gtk::manage(new TextOrIcon("wand.svg", M("MAIN_TAB_EFFECTS"),
                                       M("MAIN_TAB_EFFECTS_TOOLTIP")));
...
void ToolPanelCoordinator::addfavoritePanel(Gtk::Box *where,
                                            FoldableToolPanel *panel, int level)
{
    // auto name = panel->getToolName();
    // auto it = std::find(options.favorites.begin(), options.favorites.end(),
    // name); if (it != options.favorites.end()) {
...
void ToolPanelCoordinator::imageTypeChanged(bool isRaw, bool isBayer,
                                            bool isXtrans, bool isMono)
{
    if (isRaw) {
        if (isBayer) {
                                        ...
                                        const Glib::ustring &descr)
{
    ...
    if (event == rtengine::EvPhotoLoaded ||
        ...
        gradient->updateGeometry(
            ...
            params->gradient.feather, params->gradient.degree, fw, fh);
                                         ...
                                         bool fromLastSave)
{
    ...
    // Derive the effective changes, if it's a profile change, to prevent slow
    ...
    if (event != rtengine::EvPhotoLoaded) {
        ...
        filterRawRefresh =
            (params->raw == mergedParams.raw) &&
            (params->lensProf == mergedParams.lensProf) &&
            (params->filmNegative == mergedParams.filmNegative) &&
            (params->wb == mergedParams.wb);
    }
...
void ToolPanelCoordinator::readOptions() { crop->readOptions(); }
...
void ToolPanelCoordinator::rotateSelectionReady(double rotate_deg,
                                                Thumbnail *thm)
{
    toolBar->setTool(TMHand);
...
ToolBar *ToolPanelCoordinator::getToolBar() const { return toolBar; }
...
rtengine::RawImage *ToolPanelCoordinator::getFF()
{
    ...
    if (imd) {
        ...
        // because unused double shutter = imd->getShutterSpeed(); temporarilly
...
void ToolPanelCoordinator::saveInputICCReference(const Glib::ustring &fname,
                                                 bool apply_wb)
{
    if (ipc) {
    ...
    const LUTu &histLRETI)
{
    toneCurve->updateCurveBackgroundHistogram(
        ...
        histGreen, histBlue, histLuma, histLRETI);
    lcurve->updateCurveBackgroundHistogram(
        histToneCurve, histLCurve, histCCurve, histLCAM, histCCAM, histRed,
        histGreen, histBlue, histLuma, histLRETI);
    rgbcurves->updateCurveBackgroundHistogram(
        histToneCurve, histLCurve, histCCurve, histLCAM, histCCAM, histRed,
        histGreen, histBlue, histLuma, histLRETI);
}

void ToolPanelCoordinator::foldAllButOne(Gtk::Box *parent,
                                         FoldableToolPanel *openedSection)
{
    ...
    for (auto toolPanel : toolPanels) {
        if (toolPanel->getParent() != nullptr) {
            ...
            if (currentTP->getParent() == parent) {
                // Section in the same tab, we unfold it if it's not the one
...
bool ToolPanelCoordinator::handleShortcutKey(GdkEventKey *event)
{
    ...
    // bool ctrl = event->state & GDK_CONTROL_MASK;  temporarily removed because
    // unused bool shift = event->state & GDK_SHIFT_MASK;   temporarily removed
    // because unused
    ...
    if (alt) {
        ...
        case GDK_KEY_u:
            if (favoritePanelSW) {
                toolPanelNotebook->set_current_page(
                    toolPanelNotebook->page_num(*favoritePanelSW));
            }
            return true;
        ...
        case GDK_KEY_d:
            toolPanelNotebook->set_current_page(
                toolPanelNotebook->page_num(*detailsPanelSW));
            return true;

        case GDK_KEY_c:
            toolPanelNotebook->set_current_page(
                toolPanelNotebook->page_num(*colorPanelSW));
            return true;

        case GDK_KEY_t:
            toolPanelNotebook->set_current_page(
                toolPanelNotebook->page_num(*transformPanelSW));
            return true;

        case GDK_KEY_r:
            toolPanelNotebook->set_current_page(
                toolPanelNotebook->page_num(*rawPanelSW));
            return true;

        case GDK_KEY_f:
            toolPanelNotebook->set_current_page(
                toolPanelNotebook->page_num(*effectsPanelSW));
            return true;

        case GDK_KEY_x:
            toolPanelNotebook->set_current_page(
                toolPanelNotebook->page_num(*localPanelSW));
            return true;

        case GDK_KEY_m:
            toolPanelNotebook->set_current_page(
                toolPanelNotebook->page_num(*metadata));
            return true;
        }
    }
...
void ToolPanelCoordinator::autoExpChanged(double expcomp, int bright, int contr,
                                          int black, int hlcompr,
                                          int hlcomprthresh, bool hlrecons)
{
    // exposure->autoExpChanged(expcomp, bright, contr, black, hlcompr,
    // hlcomprthresh, hlrecons);
}

void ToolPanelCoordinator::autoMatchedToneCurveChanged(
    const std::vector<double> &curve, const std::vector<double> &curve2)
{
...
void ToolPanelCoordinator::setToolShortcutManager(ToolShortcutManager *mgr)
{
    ...
    // static_cast<TPScrolledWindow
    // *>(favoritePanelSW)->setToolShortcutManager(mgr);
    static_cast<TPScrolledWindow *>(exposurePanelSW)
        ->setToolShortcutManager(mgr);
    static_cast<TPScrolledWindow *>(detailsPanelSW)
        ->setToolShortcutManager(mgr);
    static_cast<TPScrolledWindow *>(colorPanelSW)->setToolShortcutManager(mgr);
    static_cast<TPScrolledWindow *>(transformPanelSW)
        ->setToolShortcutManager(mgr);
    ...
    static_cast<TPScrolledWindow *>(effectsPanelSW)
        ->setToolShortcutManager(mgr);
}
